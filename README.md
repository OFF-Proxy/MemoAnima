...
- **Keyboard shortcuts**: 58 commands can be reassigned (2 built-in sets plus up to 8 of your own), including "open settings" (Ctrl+, by default). Matching is by physical key position (`e.code`).
...
- **ショートカットキー**: 58コマンドの割り当てを変更可能（組み込み2種＋自作8組まで）。「⚙ 設定を開く」（既定 Ctrl+,）を含む。判定は物理キー位置（`e.code`）。

...
// ---------- GIF 固定パレット ----------
const mkSrc = (frames: Uint8ClampedArray[]) => ({ count: frames.length, fps: 8, loop: true, getFrameRgba: (i: number) => frames[i] });
...
const setPx = (f: Uint8ClampedArray, i: number, r: number, g: number, b: number, a = 255) => { f[i * 4] = r; f[i * 4 + 1] = g; f[i * 4 + 2] = b; f[i * 4 + 3] = a; };
{
  ...
  const white = collectGifPalette(mkSrc([f]), true)!;
  const trans = collectGifPalette(mkSrc([f]), false)!;
...
}
{
  ...
  const pal = collectGifPalette(mkSrc([f0, f1]), true)!;
...
}
{
  ...
  check("半透明 alpha があれば null（従来経路）", collectGifPalette(mkSrc([f]), true) === null);
}
{
  // 決定性: 同じ入力 → 同じ配列
  const f = frame();
  for (let i = 0; i < PIXELS; i++) setPx(f, i, (i * 7) % 200, (i * 13) % 200, (i * 17) % 50);
  const a = collectGifPalette(mkSrc([f]), true), b = collectGifPalette(mkSrc([f]), true);
